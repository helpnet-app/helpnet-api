...
export class Create {
  ...
  async execute(organizationId: string, newProgram: ProgramToCreateDto, pictureLink: string) {
    // Assuming you're passing the base64 string of the image as a parameter named 'image'
    const createdProgram = await this.service.create(organizationId, newProgram, pictureLink);
    if (!createdProgram) {
